<x-layout>
    <div class="w-full h-full grid grid-cols-[15%,1fr] gap-2 bg-[#f6f6f9]">
        ...
        <main class="ml-17rem w-full">
            <div>
                <h1 class="text-2xl font-semibold mt-24 text-center">Create Service For {{$business->business_name}}</h1>
                <div>
                    
                </div>
                <div class="bg-[#fff] p-[2rem] border w-[50%] rounded-md hover:shadow-xl transition-all mt-3 mx-auto">
                    <div class="table_container">
                        <form class="max-w-md mx-auto" method="POST" action="{{route('store_service',$business->id)}}" enctype="multipart/form-data">
                            @csrf
