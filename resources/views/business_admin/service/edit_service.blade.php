<x-layout>
    <div class="w-full h-full grid grid-cols-[15%,1fr] gap-2 bg-[#f6f6f9]">
        ...
        <main class="ml-17rem w-full">
            <div>
                <h1 class="text-2xl font-semibold mt-24 text-center">Edit {{$service->service_name}} Information</h1>
                <div>
                    
                </div>
                <div class="bg-[#fff] p-[2rem] border w-[50%] rounded-md hover:shadow-xl transition-all mt-3 mx-auto">
                    <div class="table_container">
                        <form class="max-w-md mx-auto" method="POST" action="{{route('update_service',$service->id)}}">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="relative z-0 w-full mb-5 group">
                                <div class="w-full">
                                    ...
                                    <textarea rows="3" name="description" id="description" placeholder="Write something here" 
                                        class="appearance-none block w-full  text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">{{$service->description}}</textarea>
                                </div>
                                    <x-forms.error></x-forms.error>
                            </div>
                            {{-- <div class="relative z-0 w-full mb-5 group">
